...
router.delete('/', async (req: Request, res: Response) => {
    try {
        ...
        const tel: any = await removeEmail(data);
        res.send(tel);
    } catch (error) {
        console.log(error);
...
router.post('/', async (req: Request, res: Response) => {
    try {
        const decoded: any = req.decoded;
        const data: any = req.body;
        const tel: any = await saveEmail(decoded.name, data);
        res.send(tel);
    } catch (error) {
        console.log(error);
        res.send({ ok: false, message: 'Save Error' });
...
async function removeEmail(data) {
    ...
    if (!validate.valid) {
        ...
        if (rs.ok) {
            ...
            rs.rows.forEach(function (d) {
                batch.find({ 'health_id': d.health_id, 'email': d.email }).deleteOne()
            });
...
async function saveEmail(name, data) {
    ...
    if (!validate.valid) {
        ...
        if (rs.ok) {
            let batch = PersonInfo.collection.initializeOrderedBulkOp();
            rs.rows.forEach(function (d) {
                batch.find({ 'health_id': d.health_id, 'email': d.email }).upsert().replaceOne(
                    { 'health_id': d.health_id, 'email': d.email, 'source': name }
                );
            });
            return new Promise((resolve, reject) => {
                batch.execute(function (err, result) {
